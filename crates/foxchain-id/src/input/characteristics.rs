...
fn detect_encoding(input: &str) -> (Vec<EncodingType>, Option<String>) {
    ...
    if is_base58(input) && input.len() >= 35 && (input.starts_with('1') || input.starts_with('3') || input.starts_with('5')) {
    ...
    if is_base58(input) && !encodings.contains(&EncodingType::Base58Check) && !encodings.contains(&EncodingType::SS58) {
...
fn extract_prefixes(input: &str) -> Vec<String> {
    ...
    length_prefixes.into_iter().chain(pattern_prefixes).collect()
}
...
mod tests {
    ...
    fn test_extract_bitcoin_bech32_characteristics() {
        ...
        assert!(chars.encoding.contains(&EncodingType::Bech32) || chars.encoding.contains(&EncodingType::Bech32m));
    }
    ...
    fn test_extract_characteristics_bitcoin_p2pkh() {
        ...
        assert!(chars.encoding.contains(&EncodingType::Base58Check) || chars.encoding.contains(&EncodingType::Base58));
        assert!(chars.prefixes.contains(&"1".to_string()));
        assert_eq!(chars.char_set, CharSet::Base58);
    }
    ...
    fn test_extract_characteristics_cosmos() {
        ...
        if chars.encoding.contains(&EncodingType::Bech32) || chars.encoding.contains(&EncodingType::Bech32m) {
    ...
    fn test_extract_characteristics_solana() {
        ...
        assert!(chars.encoding.contains(&EncodingType::Base58) || chars.encoding.contains(&EncodingType::Base58Check));
